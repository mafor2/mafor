...
module gde_coagulation
  ...
  subroutine coagulation(temp,DTIME,DPA,VPT,N,MASS,IMAX, &
...
! Initialize coagulation fluxes
    ...
    ! VTOTB is always greater than v_k = VPT(O,L).
    !------------------------------------------------------------------
...
! Source terms are updated by removal of particles by coagulation.
    ...
    !      v_j: VPT(O,J)
    !      v_k: VPT(M,I)
    ! because by definition v_j < v_k+1 - vk, f_kjk is approximately:
    !    f_kjk = v_k/VTOTB
    ! for v_j > v_k+1, f_kjk converges to 0.
    !------------------------------------------------------------------
          do I=1,IMAX
...
!MSK 2025-01-16          ! do O=M,MM
  ...
  subroutine coagulation_coeff(temp,rp0in,Dfracin,ROOP,DPA,coag,IMAX)
    !----------------------------------------------------------------------
    !
    !****
    !
    ...
    !real( dp), parameter :: rp0in    = 2.50e-9  ! radius of primary spherules [m]
    !real( dp), parameter :: Dfracin  = 2.50     ! fractal dimension           [-]
    ! Jacobson and Seinfeld (2004)
    real( dp), parameter :: sootfrac = 0.50      ! soot mass fraction
    !real( dp), parameter :: rp0in    = 13.50e-9  ! radius of primary spherules [m]
    !real( dp), parameter :: Dfracin  = 1.70      ! fractal dimension           [-]
    
    integer :: I,J,M,O
    integer :: n,ii

    ! convert radius of primary spherules from [nm] to [m]
    if ( (ICOAG.eq.2).or.(ICOAG.eq.5) ) rp0m=rp0in*1.e-9
     ...
     do I=1,IMAX
       ...
       ! Collision radius becomes fractal radius rc = rf
          nspher = ( 4*pi*(DPA(M,I)*0.5)**3./3 ) /            &
                   ( 4*pi*rp0m**3./3 )
          DPAC(M,I) = 2.* rp0m * nspher**(1./Dfracin)
       ...
       ! Jacobson and Seinfeld (2004), Equation (3)
          ra1   = min( (1.0+0.67_dp*(nspher-1.0_dp) ),   &
                       (Dfracin*nspher**(2.0_dp/Dfracin)/3.0_dp) )
          rarea = rp0m * sqrt( max( (nspher**(2.0_dp/3.0_dp)),ra1 ) )
          rmobil = max( (DPAC(M,I)*0.5/(log(DPAC(M,I)*0.5/rp0m)+1.0_dp) ), &
                        (DPAC(M,I)*0.5*((Dfracin-1.0_dp)/2.0_dp)**0.7_dp) )
          DPAM(M,I) = 2.0_dp*max( rmobil,rarea )
          if ( DPA(M,I)*0.5_dp .lt. rp0m ) then
            DPAM(M,I) = DPAC(M,I)
          endif
!MSK 2025-01-19        DPAM(M,I) =max(DPAM(M,I), DPAC(M,I))
        !write(6,*) 'dp_mobi',m,i,rp0m,Dfracin,DPA(M,I),DPAC(M,I),DPAM(M,I)
       else
          DPAC(M,I) = DPA(M,I)
          DPAM(M,I) = DPA(M,I)
       endif
     ...
     do I=1,IMAX
      do O=1,MMAX
       do J=1,IMAX
         ...
         ! Lee and Adams (2012), Coagulation vs. Analytical Solution
         ! Size-independent constant coagulation kernel
         ! A Fast and Efficient Version of the TwO-Moment Aerosol Sectional 
         ! (TOMAS) Global Aerosol Microphysics Model, Aerosol Science and
         ! Technology, 46:6, 678-689, DOI: 10.1080/02786826.2011.643259
         if (ICOAG.eq.6) then
           coag(M,O,I,J)= 10.0e-15
         endif
